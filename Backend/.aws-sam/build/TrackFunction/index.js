...
const USERS_TABLE = process.env.USERS_TABLE;   // PK = phone
...
export const handler = async (event) => {
  ...
  if (method === "OPTIONS") return j(200, { ok: true }, reqId, "cors_ok", "preflight");
  ...
  if (route.endsWith("/api/track/init") && method === "POST") {
    ...
    // (A) Submissions: ONLY submission metadata (no user object)
    ...
    // (B) Users: full user profile (PK = phone)
    if (USERS_TABLE) {
      const phoneKey = normalizePhone(user?.phone || "");
      if (!phoneKey) {
        slog(reqId, "users_put_skip_no_phone");
      } else {
        slog(reqId, "users_upsert_start", { table: USERS_TABLE, phone: phoneKey });

        // Create-or-update user. If new, set created_at; always bump last_seen.
        await ddb.update({
          TableName: USERS_TABLE,
          Key: { phone: phoneKey },
          UpdateExpression:
            "SET #n = :n, email = :e, company = :c, country = :cc, role = :r, " +
            "created_at = if_not_exists(created_at, :now), last_seen = :now " +
            "ADD submissions :one",
        ...
        slog(reqId, "users_upsert_done", { phone: phoneKey });
      }
    }
